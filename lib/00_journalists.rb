...
def calcul_longueur (array_input)
    count = 0
    array_input.each    { |i|
...
def calcul_majuscule (array_input)
    majuscule = 0
    array_input.each    { |i|
...
def find_epenser (array_input)
    array_input.each { |i|
        if i == "@epenser"
            resultat_epenser = array_input.index(i) +1
            return resultat_epenser
        end
    }
end

def sort_handles(array_input)
    counter = Hash.new(0)
    array_input.each do |handle|
        counter[handle.length] += 1
    end
    return counter
end

def presentation_counter (groupement)
    groupement.each { |key, value|
        puts "Il y a #{value} entrée de #{key} caractères"
    }
end 

def programme (array_twitter)
    puts "Combien y a-t-il de handle dans cette array ?"
    puts "Dans cette array, il y a #{array_twitter.count} entrées"
    puts
    puts "Quelle est le handle le plus court de cette liste ?"
    puts "Le handle le plus court de cette liste est #{array_twitter.sort_by(&:length)[0]} !"
    puts
    puts "Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
    puts "Il y a #{count = calcul_longueur(array_twitter)} handle de 5 caractères"
    puts
    puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
    puts "Il y a #{count = calcul_majuscule(array_twitter)} handle commençant par une majuscule"
    puts 
    puts "Trie la liste de handle par ordre alphabétique."
    puts 
    puts "--------------------------------- TRIAGE PAR ORDRE ALPHABETIQUE ----------------------------------------------"
    puts 
    puts "#{array_twitter = array_twitter.sort}"
    puts 
    "-------------------------------------------------------------------------------------------------------------------"
    puts 
    puts "Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)"
    puts
    puts "--------------------------------- TRIAGE PAR ORDRE DE TAILLE -------------------------------------------------"
    puts 
    puts "#{array_twitter = array_twitter.sort_by(&:length)} !"
    puts 
    "-------------------------------------------------------------------------------------------------------------------"
    puts 
    puts "Quelle est la position dans l'array de la personne @epenser ?"
    resultat_epenser =  find_epenser (array_twitter)
    puts "Le handle contenant @epenser est a la #{resultat_epenser}ème position"
    puts 
    ...
    groupement =  sort_handles(array_twitter)
    presentation_counter (groupement)

end

programme (array_twitter)